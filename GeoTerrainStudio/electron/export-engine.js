...
const geotiff_writer_1 = require("./geotiff-writer");
// ─── Tile Math ────────────────────────────────────────────────
const TILE_SIZE = 256;
...
function chooseZoom(bounds, targetSize, maxZoom = 19) {
...
async function mergeImageryTiles(tiles, range) {
    ...
    for (const tile of tiles) {
        ...
        const { data, info } = await (0, sharp_1.default)(tile.buffer)
            .raw()
            .ensureAlpha()
            .toBuffer({ resolveWithObject: true });
...
async function cropImagery(merged, fullW, fullH, bounds, range, zoom) {
    ...
    const width = Math.round(pxEast - pxWest);
    const height = Math.round(pxSouth - pxNorth);
    const cropped = await (0, sharp_1.default)(merged, { raw: { width: fullW, height: fullH, channels: 4 } })
...
async function resizeImagery(buffer, srcW, srcH, dstW, dstH) {
    ...
    const kernel = isDownsampling ? sharp_1.default.kernel.lanczos3 : sharp_1.default.kernel.nearest;
    return (0, sharp_1.default)(buffer, { raw: { width: srcW, height: srcH, channels: 4 } })
...
async function writeHeightmapPNG(elevations, width, height, outputPath) {
    ...
    await (0, sharp_1.default)(uint16, {
...
async function writeHeightmapGeoTIFF(elevations, width, height, bounds, outputPath) {
    ...
    const buf = (0, geotiff_writer_1.writeGeoTIFF)(int16, {
        width,
        height,
        bitsPerSample: 16,
...
async function writeAlbedoPNG(rgba, width, height, outputPath) {
    await (0, sharp_1.default)(rgba, { raw: { width, height, channels: 4 } })
        .removeAlpha()
        .png({ compressionLevel: 9 })
        .toFile(outputPath);
}
async function writeAlbedoGeoTIFF(rgba, width, height, bounds, outputPath) {
    ...
    const buf = (0, geotiff_writer_1.writeGeoTIFF)(rgb, {
        width,
        height,
        bitsPerSample: 8,
...
async function executeExport(options) {
    const { sessionId, outputPath, preset, bounds, heightmapFormat, albedoFormat, heightmapSize = 1024, albedoSize = 1024, demSource = 'aws-terrarium', imagerySource = 'arcgis', imageryZoom = 0, } = options;
    ...
    for (let y = imgRange.minY; y <= imgRange.maxY; y++) {
        for (let x = imgRange.minX; x <= imgRange.maxX; x++) {
            ...
            try {
                ...
                // Create a black tile
                const black = await (0, sharp_1.default)({
    ...
    // ── Merge DEM ──────────────────────────────────────────────
    const { elevations: mergedDEM, width: demFullW, height: demFullH } = await mergeDEMTiles(demTiles, demRange);
    const { elevations: croppedDEM, width: demCropW, height: demCropH } = cropDEM(mergedDEM, demFullW, demFullH, bounds, demRange, demZoom);
