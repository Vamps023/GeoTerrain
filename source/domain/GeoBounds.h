...
inline TileCoord latLonToTile(double lat, double lon, int zoom)
{
    ...
    const double lat_rad = lat * detail::kPi / 180.0;
    TileCoord t;
    t.z = zoom;
    t.x = static_cast<int>(std::floor((lon + 180.0) / 360.0 * n));
    t.y = static_cast<int>(std::floor((1.0 - std::log(std::tan(lat_rad) + 1.0 / std::cos(lat_rad)) / detail::kPi) / 2.0 * n));
...
{
    const double n = detail::kPi - 2.0 * detail::kPi * y / std::pow(2.0, z);
    return 180.0 / detail::kPi * std::atan(0.5 * (std::exp(n) - std::exp(-n)));
}
