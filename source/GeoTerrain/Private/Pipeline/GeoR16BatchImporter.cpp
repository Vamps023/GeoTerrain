...
<<<<<<< HEAD
    ...
    //  "Pair != nullptr" assertion in Map.h when it does FindChecked().
    //
    //  The correct pattern (proven in GeoLandscapeImporter::Import) is to
    //  generate a fresh GUID, use it as the key in the heightmap/layer data
    //  maps, and pass it to Import().  Import() handles all internal GUID
    //  registration itself.
