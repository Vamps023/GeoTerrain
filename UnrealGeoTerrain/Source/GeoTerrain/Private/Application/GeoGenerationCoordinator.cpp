...
void FGeoGenerationCoordinator::RunInternal(FGeoGenerationRequest Request)
{
    ...
    for (int32 Idx = 0; Idx < Total; ++Idx)
    {
        ...
        TileCfg.UrlTemplate      = Request.Sources.Tiles.UrlTemplate;
        const FString AlbedoPath = FPaths::Combine(OutputDir, TEXT("albedo.tif"));

        // We store output path inside the config temporarily
        FGeoTileDownloader TileDL;
        FGeoTileSettings TileCfgOut = TileCfg;
        auto TileResult = TileDL.Download(Chunk.Bounds, TileCfgOut, Context);
